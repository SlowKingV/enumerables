...
puts greet.my_all? { |obj| (obj[0].is_a? Symbol) && (obj[1] =~ /^h/) } #Should print 'true' since every key is a Symbol and every value starts with an 'h'
puts ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
puts greet.my_any? { |obj| (obj[0] == :question) && (obj[1] =~ /\?$/) } #Should print 'true' since at least one of its keys is called :question and its value ends with '?'
puts ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
...
puts greet.my_none? { |obj| (obj[0].length > 5) && (obj[1] =~ /[^\!\?]$/) } #Should print 'false' since at least one of its key names is longer than 5 and ends without '!' or '?'
puts ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
